...
public class CalloutInvoice extends CalloutEngine {
	 ...
	 */
	public String docType(Ctx ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue) {
		...
		if ((oldC_DocType_ID == null || oldC_DocType_ID.intValue() == 0) && oldValue != null) { // get old Target
			oldC_DocType_ID = (Integer) oldValue;
		}
		String sql = "SELECT d.HasCharges,'N',d.IsDocNoControlled,"
				+ "s.CurrentNext, d.DocBaseType, s.CurrentNextSys, s.AD_Sequence_ID " + "FROM C_DocType d "
				+ "LEFT OUTER JOIN AD_Sequence s ON (d.DocNoSequence_ID=s.AD_Sequence_ID) " + "WHERE C_DocType_ID=?"; // 1
		try {
	...
	public String bPartner(Ctx ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		try {
			...
			if (rs.next()) {
				...
				if (C_BPartner_ID.toString()
						.equals(ctx.getContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "C_BPartner_ID"))) {
					String loc = ctx.getContext(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO,
	 ...
	 * PriceList, PriceStd, PriceLimit, C_Currency_ID, EnforcePriceLimit - UOM Calls
	 * Tax
	 * 
	 * @param ctx      context
	 * @param WindowNo window no
	...
	public String product(Ctx ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		if (ctx.getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "M_Product_ID") == M_Product_ID
				.intValue()
				&& ctx.getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO,
						"M_AttributeSetInstance_ID") != 0) {
			mTab.setValue("M_AttributeSetInstance_ID", Integer.valueOf(
					ctx.getContextAsInt(EnvConstants.WINDOW_INFO, EnvConstants.TAB_INFO, "M_AttributeSetInstance_ID")));
		} else {
		...
		MProductPricing pp = new MProductPricing(ctx.getAD_Client_ID(), ctx.getAD_Org_ID(), M_Product_ID.intValue(),
		...
		// since UOM specified is changed (as a result of tab out from product),
		// recalculate QtyEntered and QtyInvoiced based on new UOM
		BigDecimal QtyEntered = (BigDecimal) mTab.getValue("QtyEntered");
		if (newC_UOM_ID != 0) {
			BigDecimal QtyEntered1 = QtyEntered.setScale(MUOM.getPrecision(ctx, newC_UOM_ID), BigDecimal.ROUND_HALF_UP);
			if (QtyEntered.compareTo(QtyEntered1) != 0) {
				log.fine("Corrected QtyEntered Scale UOM=" + newC_UOM_ID + "; QtyEntered=" + QtyEntered + "->"
						+ QtyEntered1);
		...
		// UOM Conversion
		BigDecimal QtyInvoiced = MUOMConversion.convertProductFrom(ctx, M_Product_ID.intValue(), newC_UOM_ID,
				QtyEntered);
		if (QtyInvoiced == null) {
	...
	/**
	 * Invoice Line - Charge. - updates PriceActual from Charge - sets PriceLimit,
	...
	public String amt(Ctx ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		if ((mField.getColumnName().equals("QtyInvoiced") || mField.getColumnName().equals("QtyEntered")
			...
			boolean IsSOTrx = ctx.getContext(WindowNo, "IsSOTrx").equals("Y");
			MProductPricing pp = new MProductPricing(ctx.getAD_Client_ID(), ctx.getAD_Org_ID(), M_Product_ID,
			...
			//
			log.fine("amt - QtyChanged -> PriceActual=" + pp.getPriceStd() + ", PriceEntered=" + PriceEntered
		 ...
		 * (mField.getColumnName().equals("Discount")) { PriceActual = new BigDecimal
		 * ((100.0 - Discount.doubleValue()) / 100.0 * PriceList.doubleValue()); if
		 * (PriceActual.scale() > StdPrecision) PriceActual =
		 ...
		 * mTab.setValue("PriceActual", PriceActual); mTab.setValue("PriceEntered",
		 * PriceEntered); } // calculate Discount else { if (PriceList.intValue() == 0)
		 * Discount = Env.ZERO; else Discount = new BigDecimal ((PriceList.doubleValue()
		 ...
		 * log.fine("amt = PriceEntered=" + PriceEntered + ", Actual" + PriceActual + ",
		 * Discount=" + Discount); /*
		 */
	...
	private boolean isTaxIncluded(int WindowNo) {
		...
		if (ss.length() == 0) {
			...
			ss = DB.getSQLValueString(null, "SELECT IsTaxIncluded FROM M_PriceList WHERE M_PriceList_ID=?",
					M_PriceList_ID);
			if (ss == null) {
				ss = "N";
			}
	...
	public String qty(Ctx ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		// No Product
		/*
		 * if (M_Product_ID == 0) { QtyEntered =
		 * (BigDecimal)mTab.getValue("QtyEntered"); mTab.setValue("QtyInvoiced",
		 * QtyEntered); }
		 */
		...
		/* else */ if (mField.getColumnName().equals("C_UOM_ID")) {
			...
			if (C_UOM_To_ID != 0) {
				BigDecimal QtyEntered1 = QtyEntered.setScale(MUOM.getPrecision(ctx, C_UOM_To_ID),
			...
			log.fine("qty - UOM=" + C_UOM_To_ID + ", QtyEntered/PriceActual=" + QtyEntered + "/" + PriceActual + " -> "
					+ conversion + " QtyInvoiced/PriceEntered=" + QtyInvoiced + "/" + PriceEntered);
		...
		// QtyEntered changed - calculate QtyInvoiced
			...
			boolean conversion = QtyEntered.compareTo(QtyInvoiced) != 0;
			log.fine("qty - UOM=" + C_UOM_To_ID + ", QtyEntered=" + QtyEntered + " -> " + conversion + " QtyInvoiced="
					+ QtyInvoiced);
			ctx.setContext(WindowNo, "UOMConversion", conversion ? "Y" : "N");
			mTab.setValue("QtyInvoiced", QtyInvoiced);
		}
		// QtyInvoiced changed - calculate QtyEntered (should not happen)
			...
			log.fine("qty - UOM=" + C_UOM_To_ID + ", QtyInvoiced=" + QtyInvoiced + " -> " + conversion + " QtyEntered="
					+ QtyEntered);
			ctx.setContext(WindowNo, "UOMConversion", conversion ? "Y" : "N");
			mTab.setValue("QtyEntered", QtyEntered);
		}

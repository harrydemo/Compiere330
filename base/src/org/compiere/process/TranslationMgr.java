...
public class TranslationMgr {
	 ...
	 * @param translationLevel optional translation level
	 *                         TranslationImportExport.TranslationLevel_*
	 * @return status message
	 */
	public String exportTrl(String directory, String trlTableName, String translationLevel) {
		...
		try {
			...
			// <!DOCTYPE compiereTrl PUBLIC "-//ComPiere, Inc.//DTD Compiere Translation
			// 1.0//EN" "http://www.compiere.org/dtd/compiereTrl.dtd">
			...
			for (String element : trlColumns) {
				sql.append(", t.").append(element).append(",o.").append(element).append(" AS ").append(element)
						.append("O");
			}
			//
			sql.append(" FROM ").append(tableName).append(" t").append(" INNER JOIN ").append(baseTableName)
			...
			if (m_AD_Client_ID >= 0) {
				sql.append(haveWhere ? " AND " : " WHERE ").append("o.AD_Client_ID=").append(m_AD_Client_ID);
				haveWhere = true;
			}
				...
				for (String element : trlColumns) {
					if (!baseTableName.equals("AD_PrintFormatItem")) {
						if (translationLevel.equals(TranslationImportExport.TranslationLevel_LabelOnly)
								&& !((element.indexOf("Name") != -1) || element.equals("MsgText"))) {
							continue;
						}
						if (translationLevel.equals(TranslationImportExport.TranslationLevel_LabelDescriptionOnly)
								&& !((element.indexOf("Name") != -1) || (element.indexOf("Description") != -1)
										|| element.equals("MsgText") || element.equals("MsgTip")
										|| element.equals("PrintName") || element.equals("RegionName"))) {
							continue;
						}
					}
	...
	private String[] getTrlColumns(String Base_Table) {
		...
		sql = "SELECT ColumnName " + "FROM AD_Column c" + " INNER JOIN AD_Table t ON (c.AD_Table_ID=t.AD_Table_ID) "
				+ "WHERE t.TableName=?" + " AND c.AD_Reference_ID IN (10,14) "
	...
	private String getScopeSQL(String baseTableName) {
		if (Util.isEmpty(baseTableName) || (m_ExportScope == null)
		...
		case "AD_Table":
			return "1=2";
		case "AD_Window":
			return "o.AD_Window_ID IN (SELECT t.AD_Window_ID FROM AD_Tab tab" + " INNER JOIN AD_Table tt ON (tab.AD_Table_ID=tt.AD_Table_ID) " + "WHERE tt.AccessLevel <> '4')";
		case "AD_Tab":
			return "EXISTS (SELECT * FROM AD_Table tt "	+ "WHERE o.AD_Table_ID=tt.AD_Table_ID AND tt.AccessLevel <> '4')";
		case "AD_Field":
			return "o.AD_Tab_ID IN (SELECT AD_Tab_ID FROM AD_Tab tab" + " INNER JOIN AD_Table tt ON (tab.AD_Table_ID=tt.AD_Table_ID) " + "WHERE tt.AccessLevel <> '4')";
		case "AD_Element":
			return "o.AD_Element_ID IN (SELECT AD_Element_ID FROM AD_Column c"	+ " INNER JOIN AD_Table tt ON (c.AD_Table_ID=tt.AD_Table_ID) " + "WHERE tt.AccessLevel <> '4')";
		case "AD_Process":
	...
	/**************************************************************************
	 * Validate Language. - Check if AD_Language record exists - Check Trl table
	 * records
	 * 

...
public class TranslationImportExport extends SvrProcess {
	...
	protected void prepare() {
		...
		for (ProcessInfoParameter element : para) {
			...
			case "p_AD_Table_ID":
				...
				break;
			case "TranslationLevel":
				p_TranslationLevel = name;
				break;
			case "Directory":
				p_Directory = name;
				break;
	...
	protected String doIt() throws Exception {
		log.info("AD_Language=" + p_AD_Language + ",Mode=" + p_ImportExport + ",Scope=" + p_ExportScope
				+ ",AD_Table_ID=" + p_AD_Table_ID + ",Level=" + p_TranslationLevel + ",Directory=" + p_Directory);
		//
		...
		if (p_AD_Table_ID == 0) {
			...
			for (MTable table : tables) {
				...
				msg = null;
				msg = importMode ? trl.importTrl(p_Directory, tableName)
		...
		} else // single table
		{
			...
			msg = null;
			msg = importMode ? trl.importTrl(p_Directory, table.getTableName())

...
// comboZoom.setToolTipText(Msg.getMsg(m_ctx, "Zoom"));
		...
		ppp.add(labelDrill, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(0, 0, 1, 0), 0, 0));
		ppp.add(comboDrill, new GridBagConstraints(0, 1, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
		comboDrill.setToolTipText(ss);
		// Format
		ss = Msg.getElement(m_ctx, "AD_PrintFormat_ID");
		labelReport.setText(ss + ":");
		ppp.add(labelReport, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE,
				new Insets(0, 3, 1, 0), 0, 0));
		ppp.add(comboReport, new GridBagConstraints(1, 1, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE,
...
// comboZoom.addActionListener(this);
		...
		String sql = "SELECT t.AD_Table_ID, t.TableName, e.PrintName, NULLIF(e.PO_PrintName,e.PrintName) "
				+ "FROM AD_Column c " + " INNER JOIN AD_Column used ON (c.ColumnName=used.ColumnName)"
				...
				+ "WHERE c.AD_Table_ID=? AND c.IsKey='Y' " + "ORDER BY 3";
		...
		if (trl) {
			sql = "SELECT t.AD_Table_ID, t.TableName, et.PrintName, NULLIF(et.PO_PrintName,et.PrintName) "
					...
					+ "WHERE c.AD_Table_ID=? AND c.IsKey='Y'" + " AND et.AD_Language=? " + "ORDER BY 3";
		}
		try {
	...
	private void fillComboReport(int AD_PrintFormat_ID) {
		...
		if (AD_Process_ID != 0) {
			list = MPrintFormat.getPrintFormatsForProcess(m_ctx, m_reportEngine.getPrintInfo().getAD_Process_ID(),
					MRole.SQL_RO);
		} else {
			list = MPrintFormat.getPrintFormatsByTable(m_ctx, m_reportEngine.getPrintFormat().getAD_Table_ID(),
					MRole.SQL_RO);
		}
	...
	public void revalidate() {
		...
		centerScrollPane
				.setPreferredSize(new Dimension(m_viewPanel.getPaperWidth() + 30, m_viewPanel.getPaperHeight() + 15));
		centerScrollPane.getViewport().setViewSize(new Dimension(m_viewPanel.getPaperWidth() + 2 * View.MARGIN,
		...
		sb.append(m_viewPanel.getPaper().toString(m_ctx)).append(" - ").append(Msg.getMsg(m_ctx, "DataCols"))
				.append("=").append(m_reportEngine.getColumnCount()).append(", ").append(Msg.getMsg(m_ctx, "DataRows"))
				.append("=").append(m_reportEngine.getRowCount());
	...
	private void createMenu() {
		...
		AEnv.addMenuItem("ScreenShot", null, KeyStroke.getKeyStroke(KeyEvent.VK_PRINTSCREEN, Event.SHIFT_MASK), mFile,
				this);
		...
		AEnv.addMenuItem("Exit", null, KeyStroke.getKeyStroke(KeyEvent.VK_X, Event.SHIFT_MASK + Event.ALT_MASK), mFile,
				this);
		...
		AEnv.addMenuItem("InfoBPartner", null,
				KeyStroke.getKeyStroke(KeyEvent.VK_I, Event.SHIFT_MASK + Event.CTRL_MASK), mView, this);
		AEnv.addMenuItem("InfoAccount", null, KeyStroke.getKeyStroke(KeyEvent.VK_I, Event.ALT_MASK + Event.CTRL_MASK),
				mView, this);
...
// else
	...
	private void setPageIndicator(int page) {
		...
		StringBuffer sb = new StringBuffer(Msg.getMsg(m_ctx, "Page")).append(" ").append(m_pageNo)
				.append(m_viewPanel.getPageInfo(m_pageNo)).append(" ").append(Msg.getMsg(m_ctx, "of")).append(" ")
				.append(m_pageMax).append(m_viewPanel.getPageInfoMax());
	...
	private void cmd_find() {
		...
		if (!Env.isBaseLanguage(Env.getCtx(), "AD_Tab")) {
			sql = "SELECT Name, TableName FROM AD_Tab_vt WHERE AD_Tab_ID=?" + " AND AD_Language='"
		...
		if (findFields == null) { // No Tab for Table exists
			...
			Find find = new Find(this, m_WindowNo, title, 0, AD_Table_ID, tableName, "", findFields, 1);
	...
	/**
	 * Show Translation Dialog. Translate base table entry, will be copied to trl
	 * tables if not multi-lingual
	 */
	private void cmd_translate() {
		...
		int choice = JOptionPane.showOptionDialog(this, message, title, JOptionPane.OK_OPTION,
		...
		if (Language.isBaseLanguage(AD_Language)) {
			sb.append("UPDATE AD_PrintFormatItem pfi " + "SET Name = (SELECT e.Name FROM AD_Element e, AD_Column c"
					...
					+ " WHERE e.AD_Element_ID=c.AD_Element_ID AND c.AD_Column_ID=pfi.AD_Column_ID) "
					+ "WHERE AD_PrintFormat_ID=").append(AD_PrintFormat_ID)
					.append(" AND EXISTS (SELECT * FROM AD_Element e, AD_Column c"
							+ " WHERE e.AD_Element_ID=c.AD_Element_ID AND c.AD_Column_ID=pfi.AD_Column_ID)");
		} else {
			AD_Language = "'" + AD_Language + "'";
			sb.append("UPDATE AD_PrintFormatItem pfi "
					+ "SET Name = (SELECT e.Name FROM AD_Element_Trl e, AD_Column c" + " WHERE e.AD_Language=")
					.append(AD_Language)
					...
					.append(" AND EXISTS (SELECT * FROM AD_Element_Trl e, AD_Column c" + " WHERE e.AD_Language=")
					.append(AD_Language)
					.append(" AND e.AD_Element_ID=c.AD_Element_ID AND c.AD_Column_ID=pfi.AD_Column_ID)");
		}

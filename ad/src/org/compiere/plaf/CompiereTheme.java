...
/**
 * Compiere User definable Theme (if used in Metal L&F). In other Environments,
 * it provides UI extensions (e.g. Error Color)
 *
 * @author Jorg Janke
...
public class CompiereTheme extends MetalTheme {
		...
		if (controlFont == null) {
			try {
				controlFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.controlFont", new Font(FONT_DEFAULT, Font.PLAIN, FONT_SIZE)));
			} catch (Exception e) {
		...
		if (systemFont == null) {
			try {
				systemFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.systemFont", new Font(FONT_DEFAULT, Font.PLAIN, FONT_SIZE)));
			} catch (Exception e) {
		...
		if (userFont == null) {
			try {
				userFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.userFont", new Font(FONT_DEFAULT, Font.PLAIN, FONT_SIZE)));
			} catch (Exception e) {
		...
		if (menuFont == null) {
			try {
				menuFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.menuFont", new Font(FONT_DEFAULT, Font.PLAIN, FONT_SIZE)));
			} catch (Exception e) {
	...
	protected static FontUIResource _getWindowTitleFont() {
		if (windowFont == null) {
			try {
				windowFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.windowFont", new Font(FONT_DEFAULT, Font.BOLD, FONT_SIZE + 2)));
			} catch (Exception e) {
		...
		if (smallFont == null) {
			try {
				smallFont = new FontUIResource(
						Font.getFont("swing.plaf.metal.smallFont", new Font(FONT_DEFAULT, Font.PLAIN, FONT_SIZE - 2)));
			} catch (Exception e) {
	...
	/**
	 * Parses Color into String representation. Required as SystemColors and Alpha
	 * Colors have different formats
	 * 
	 * @param c Color
	...
	public static String getColorAsString(Color c) {
		...
		StringBuffer sb = new StringBuffer("[r=").append(c.getRed()).append(",g=").append(c.getGreen()).append(",b=")
				.append(c.getBlue()).append(",a=").append(c.getAlpha()).append("]");
	...
	public static void reset() {
		/**
		 * Ini.remove (P_Primary1); Ini.remove (P_Primary2); Ini.remove (P_Primary3);
		 * Ini.remove (P_Secondary1); Ini.remove (P_Secondary2); Ini.remove
		 * (P_Secondary3); Ini.remove (P_Error); Ini.remove (P_Info); Ini.remove
		 * (P_Mandatory); Ini.remove (P_Inactive); Ini.remove (P_White); Ini.remove
		 * (P_Black); Ini.remove (P_Txt_OK); Ini.remove (P_Txt_Error); // Ini.remove
		 * (P_Control); Ini.remove (P_System); Ini.remove (P_User); Ini.remove
		 * (P_Small); Ini.remove (P_Window); Ini.remove (P_Menu); // CompiereColor
		...
		// Background
		// CompiereColor cc = new CompiereColor(SystemColor.control); // flat Windows
		// 212-208-200
	...
	private static FontUIResource parseFont(String information, FontUIResource stdFont) {
		...
		try {
			...
			int size = Integer
					.parseInt(information.substring(information.indexOf(",size=") + 6, information.lastIndexOf("]")));

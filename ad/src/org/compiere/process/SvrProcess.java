...
public abstract class SvrProcess implements ProcessCall {
	 ...
	 * @throws Exception if not successful e.g. throw new CompiereUserError
	...
	private void lock() {
		...
		DB.executeUpdate("UPDATE AD_PInstance SET IsProcessing='Y' WHERE AD_PInstance_ID=" + m_pi.getAD_PInstance_ID(),
				(Trx) null); // outside p_trx
	} // lock

	/**
	 * Unlock Process Instance. Update Process Instance DB and write option return
	 * message
	 */
	private void unlock() {
	...
	protected Trx get_TrxName() {
		return m_trx;
		/*
		 * if (m_trx != null) return m_trx.getTrxName(); return null;
		 */
	} // get_TrxName

...
public class UIField extends UIFieldVO {
	...
	public boolean isMandatory(CContext ctx, int WindowNo) {
		...
		if ((isKey() && columnName.endsWith("_ID")) || columnName.startsWith("Created")
				|| columnName.startsWith("Updated") || columnName.equals("Value") || columnName.equals("DocumentNo")
	...
	public boolean isDisplayed(CContext ctx, int WindowNo) {
		...
		if (ctx != null) {
			// TODO fix context
			boolean retValue = Evaluator.evaluateLogic(ctx.getCtx(WindowNo), logic);
	...
	public boolean isEditable(CContext ctx, int WindowNo, UITab tab, boolean inserting) {
		...
		if (tab.isReadOnly() || isReadOnly()) {
			log.finest(columnName + " NO - TabRO=" + tab.isReadOnly() + ", FieldRO=" + isReadOnly());
			return false;
		}
	...
	public boolean isLookup() {
		...
		// retValue = false;
			...
			retValue = true;
		} else if ((displayType == DisplayTypeConstants.Location) || (displayType == DisplayTypeConstants.Locator)
				|| (displayType == DisplayTypeConstants.Account) || (displayType == DisplayTypeConstants.PAttribute)) {
			retValue = true;
		}
		return retValue;
	} // isLookup
	...
	public boolean isButtonLookup() {
		if (getAD_Reference_ID() == DisplayTypeConstants.Button) { // not cached
			if (getColumnName().endsWith("_ID") && !getColumnName().equals("Record_ID")) {
				return true;
	...
	private Lookup createAllLookup(Ctx ctx, int WindowNo, boolean assignInfo) {
		...
		if (FieldType.isLookup(displayType)) {
			...
			lookupInfo = MLookupFactory.getLookupInfo(ml, getAD_Column_ID(), Env.getLanguage(ctx), columnName,
					getAD_Reference_Value_ID(), isParent(), getValidationCode());
			if (lookupInfo == null) {
		...
		} else if (displayType == DisplayTypeConstants.PAttribute) // not cached
		{
			...
			if (getColumnName().endsWith("_ID") && !getColumnName().equals("Record_ID")) {
				lookup = MLookupFactory.get(ctx, WindowNo, getAD_Column_ID(), DisplayTypeConstants.Search, false);
			} else if (getAD_Reference_Value_ID() != 0) {
				// Assuming List
				lookup = MLookupFactory.get(ctx, WindowNo, getAD_Column_ID(), DisplayTypeConstants.List, false);
			}
		}
		if (assignInfo) {
			m_lookupInfo = lookupInfo;
	...
	private void createDependsOnLists() {
		...
		if (CLogMgt.isLevelFiner()) {
			if (m_dependsOnValue.size() > 0) {
				log.finer("(" + columnName + ") Value " + m_dependsOnValue.toString());
			}
			if (m_dependsOnUI.size() > 0) {
				log.finer("(" + columnName + ") UI - " + m_dependsOnUI.toString());
			}
		}
	} // createDependsOnLists
	...
	private void setLocatorProperties(Lookup lookup, Ctx ctx, int windowNo, String columnName) {

		MLocatorLookup ml = (MLocatorLookup) lookup;
		try {
			String warehouseID = ctx.getContext(windowNo, "M_Warehouse_ID", true);
	...
	public ArrayList<NamePair> getAllLookupData(Ctx ctx, int windowNo) {
		...
		lookup.fillComboBox(isMandatory(), onlyValidated, onlyActive, temporary);
		ArrayList<NamePair> list = lookup.getData(isMandatory(), onlyValidated, onlyActive, temporary);
		if (list == null) {
	 ...
	 * @param windowNo window
	 * @param key      the string key value
	 * @param whether  read from cache or from table
	 * @return display or null if not found
	 */
	...
	public Object getDefault(CContext ctx, int WindowNo, UITab tab) {
		...
		if (isKey() || (displayType == DisplayTypeConstants.RowID) || FieldType.isLOB(displayType)) {
			return null;
		}
		...
		else if (X_AD_Table.ACCESSLEVEL_SystemPlusTenant.equals(AccessLevel) && ColumnName.equals("AD_Org_ID")) {
		...
		if (DefaultValue.startsWith("@SQL=")) {
			...
			if ((sqlTest.indexOf("DELETE ") != -1) && (sqlTest.indexOf("UPDATE ") != -1)
		...
		if (!DefaultValue.equals("") && !DefaultValue.startsWith("@SQL=")) {
			...
			while (st.hasMoreTokens()) {
				...
				if (variable.equals("@SysDate@") || variable.equals("@Now@")) { // System
					...
					defStr = ctx.getContext(WindowNo, variable.replace('@', ' ').trim());
		...
		if ((m_lookup != null) && !Util.isEmpty(m_lookup.getValidation())) {
			...
			for (String var : vars) {
				...
				if (!var.startsWith("#") && var.endsWith("_ID") && !var.equals(ColumnName)) {
		...
		// Button to N
		if ((displayType == DisplayTypeConstants.Button) && !ColumnName.endsWith("_ID")) {
	...
	private Object createDefault(String variable, String value) {
		...
		try {
			...
			if (FieldType.isDate(displayType)) {
				...
				String tsString = value + "2007-01-01 00:00:00.000000000".substring(value.length());
				try {
	...
	public BigDecimal convertToBigDecimal(String stringValue) {
		...
		try {
			...
			log.warning(getColumnName() + ": " + stringValue + " - " + e.toString());
		}
		return null;
	} // convertToBigDecimal
	...
	public Timestamp convertToTimestamp(String stringValue) {
		...
		try // CurrentTime format
		{
			...
			try // JDBC format
			{
				...
				log.warning(getColumnName() + ": " + stringValue + " - " + e2.toString());
			}
		}
		return null;
	} // convertToTimestamp
	...
	public boolean isParentValue(UITab tab) {
		...
		if (!FieldType.isID(getAD_Reference_ID()) || (tab.getTabNo() == 0)) {
			...
			if (LinkColumnName.length() == 0) {
				...
				m_parentValue = Boolean.valueOf(ColumnName.equals(LinkColumnName));
			}
		}
		return m_parentValue.booleanValue();
	} // isParentValue
	...
	/**
	 * Get the list of column names this field Depends On for Value. (e.g. Business
	 * Partner for a Location field)
	 * 
	 * @return list of columns
	 */
	...
	/**
	 * Get the list of column names this field impacts their UI. (Display, ReadOnly,
	 * Mandatory)
	 * 
	 * @return list of columns
	 */
	 ...
	 * 
	 * @param impacts true if tab impacted
	 */
	...
	public boolean isImpactsValue() {
		...
		return (m_impactsValue.size() > 0) || isCallout() || ((callOut != null) && (callOut.length() > 0));
	} // isImpactsValue
	...
	protected boolean isCreateMnemonic() {
		String columnName = getColumnName();
		if (isReadOnly() || columnName.equals("AD_Client_ID") || columnName.equals("AD_Org_ID")
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("UIField[").append(getAD_Field_ID()).append("-").append(getColumnName());
		if (m_lookup != null) {

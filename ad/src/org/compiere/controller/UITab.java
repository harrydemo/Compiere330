...
public class UITab extends UITabVO {
	 ...
	 */
	protected void initialize(ArrayList<UIField> fields, ArrayList<UITab> previousTabs, CContext ctx, int windowNo,
			...
			if (linkColumnName == null || linkColumnName.length() == 0) {
				...
				// Multiple Parents
					...
					for (int i = 0; i < previousTabs.size(); i++) {
						...
						String previousKeyColumn = previousTab.getKeyColumnName(); // single key
	...
	/**
	 * Create Dependency Relations. Field Logic contains the columns it depends on
	 * [displayed if Active=Y] (dependensOn - created via
	 * Field.initialize()/createDependsOnLists()) This routine calculates the Impact
	 * [Active impacts] and saves that in the field.
	 */
	...
	/**
	 * Get the list of column names this tab Depends On for UI. (Display, ReadOnly,
	 * Mandatory)
	 * 
	...
	public String getKeyColumnName() {
		...
		if (size == 0) {
			...
			log.fine("More than one KeyColumn - " + toString() + " : " + keyColumns);
		}
			...
			if (sort != null && sort.signum() != 0) {
				...
				if (index < 3) {
					if (m_orderBys[index] == null) {
						...
						log.warning("Ignored OrderBy Duplicate Position " + (index + 1) + ": " + columnName + " - "
								+ toString());
					}
				} else {
				...
				// creates Ctrl_Shift_
				{
					...
					if (mnemonic != ' ') {
						if (!m_mnemonics.contains(mnemonic)) {
							...
							log.warning(field.getColumnName() + " - Conflict - Already exists: " + mnemonic + " ("
									+ text + ")");
						}
					}
				}
	 ...
	 */
	public Object[][] executeQuery(QueryVO queryVO, HashMap<String, String> context, CContext ctx) {
		MRole role = MRole.getDefault(ctx, false);
		QueryWithBindings whereClause = getWhereClauseWithBindings(queryVO, role, context, ctx, true);
		...
		String sql1 = role.addAccessSQL(sql0.toString(), getTableName(), MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
		//
		// String[] columns = getColumnNames();

		Object[][] results = new Object[0][];
		try {
			results = QueryUtil.executeQuery(sql1, whereClause.params.toArray(), 0, MAX_QUERY_RESULT_SIZE + 1);
		...
		if (results.length > 0) {
			...
			if (session == null) {
				...
				session.queryLog(ctx.getAD_Client_ID(), ctx.getAD_Org_ID(), getAD_Table_ID(), sql1, results.length);
			}
		}
		// Results
		return results;
	 ...
	 */
	private QueryWithBindings getWhereClauseWithBindings(QueryVO queryVO, MRole role, HashMap<String, String> context,
			CContext ctx, boolean addOrderBy) {
	 ...
	 * @return
	 */
	public static String[] FORBIDDEN = { "where", "insert", "update", "delete", "select", "from" };
		...
		for (String token : tokens) {
			...
			for (String forbidden : FORBIDDEN) {
				if (forbidden.equals(token)) {
					throw new IllegalStateException("Can't use word:" + token + " and the where clause:" + where);
					// return false;
				}
			}
		}
	...
	private Query createQuery(CContext ctx, QueryVO queryVO) {
		...
		if (queryVO.restrictions != null && queryVO.restrictions.size() > 0) {
			...
			for (int i = 0; i < queryVO.restrictions.size(); i++) {
				...
				if (checkWhereClause(vo.DirectWhereClause)) {
					...
					QueryRestriction restriction = new QueryRestriction(vo.ColumnName, qCode, qCode_to, vo.InfoName,
							vo.InfoDisplay, vo.InfoDisplay_to, vo.Operator, vo.DirectWhereClause, vo.AndCondition);
		...
		// Save Query
		if (queryVO.savedQueryName != null && queryVO.savedQueryName.length() > 0 && queryVO.saveQuery
				&& query != null) {
	 ...
	 * @param currentCreated if true get recent records based on Created otherwise
	 *                       Updated
	...
	private String validateQuery(Query query) {
		...
		} else if (colName.startsWith("C_DocType") && !colName.equals("C_DocTypeGroup_ID")) {
		...
		StringBuffer result = new StringBuffer("EXISTS (SELECT * FROM ").append(tableName).append(" xx WHERE ")
				.append(query.getWhereClause(true)).append(" AND xx.").append(tabKeyColumn).append("=")
		...
		if (!newRecord && cachedRow != null) {
			...
			if (fields.size() > 0) {
				ChangeVO c = new ChangeVO(true,
						"Please refresh the row. The following fields are updated by others:" + fields);
				return c;
			}
		}
	...
	public ChangeVO newRow(CContext ctx, int windowNo) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (field.isCallout() || !Util.isEmpty(field.getCallout())) {
				...
				ChangeVO calloutChange = processCallout(ctx, windowNo, field, null, newValue);
				if (org.compiere.common.constants.Build.isDebug()) {
	...
	private PO getPO(CContext ctx, int windowNo, boolean newRecord) {
		...
		if (newRecord) {
			...
			for (String columnName : m_parentColumns) {
				po.set_ValueNoCheck(columnName, ctx.getContextAsInt(windowNo, columnName));
			}
			return po;
		}
		//
	...
	private String checkReadOnly(CContext ctx, int windowNo) {
		...
		MRole role = MRole.getDefault(ctx, false);
		if (!role.canUpdate(AD_Client_ID, AD_Org_ID, getAD_Table_ID(), Record_ID, createError)) {
	...
	private String checkMandatory(CContext ctx, int windowNo) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (field.isMandatory(ctx, windowNo)) // check context
			{
				...
				if (data == null || data.toString().length() == 0 || Null.NULLString.equals(data)) {
	 ...
	 */
	public ChangeVO fieldChanged(CContext origCtx, CContext ctx, ArrayList<UIField> changedFields, int windowNo,
		...
		if (field.isImpactsValue()) {
			...
			for (int i = 0; i < impactedColumnNames.size(); i++) {
				...
				String oldImpactedValue = origCtx.getContext(windowNo, impactedColumnName);
				...
				if (impactedField.isLookup()) {
					...
					if (impactedLookup instanceof MLocatorLookup) {
						...
						Integer M_Locator_ID = ctx.getContextAsInt(windowNo, impactedColumnName);
					...
					else if (impactedLookup.getDisplayType() == DisplayTypeConstants.Search
							|| (impactedField.getLookupInfo() != null
									&& impactedField.getLookupInfo().IsCreadedUpdatedBy)) {
			...
			ChangeVO calloutChange = processCallout(ctx, windowNo, field, oldValue, newValue);
			...
			if (calloutChange != null) {
				...
				if (calloutChange.changedFields != null) {
					...
					for (String colName : calloutChange.changedFields.keySet()) {
						...
						if (changedFields.contains(impactedField)) {
							log.info("(changed in callout)field:" + impactedField.getColumnName()
									+ " already changed. don't run again");
							continue;
						}
			...
			for (UIField impactedField : impactedFields) {
				ChangeVO cascadingChange = fieldChanged(origCtx, ctx, changedFields, windowNo, impactedField,
						origCtx.getContext(windowNo, impactedField.getColumnName()),
						ctx.getContext(windowNo, impactedField.getColumnName()));
		...
		// security is ok
		if (field.getColumnName().equals("AD_Org_ID") || field.getColumnName().equals("AD_Client_ID")) {
	...
	public void canUpdate(CContext ctx, int windowNo, ChangeVO change) {
		...
		int AD_Client_ID = Integer.parseInt(ctx.getContext(windowNo, "AD_Client_ID"));
	 ...
	 * Process Callout(s).
	 * <p>
	 * The Callout is in the string of "class.method;class.method;" If there is no
	 * class name, i.e. only a method name, the class is regarded as CalloutSystem.
	 * The class needs to comply with the Interface Callout. For a limited time, the
	 * old notation of Sx_matheod / Ux_menthod is maintained.
	 * 
	...
	private ChangeVO processCallout(CContext ctx, int windowNo, UIField field, String oldValue, String newValue) {
		...
		if (ctx.isProcessed(windowNo)) { // only active records
			log.warning("Record processed - Ignored Callout " + field.getColumnName());
		...
		if (field.isCallout()) {
			...
			processCalloutDirect(retValue, po, windowNo, field, oldValue, newValue);
		...
		while (st.hasMoreTokens()) // for each callout
		{
			...
			log.fine(field.getColumnName() + "=" + newValue + " (" + callout + ") - old=" + oldValue);
			processCalloutExternal(retValue, po, windowNo, field, oldValue, newValue, cmd);
			CThreadUtil.setCalloutActive(true);
	 ...
	 */
	private void processCalloutExternal(ChangeVO change, PO po, int windowNo, UIField field, String oldValue,
			String newValue, String cmd) {
		if (Util.isEmpty(cmd)) {
			return;
		}
		...
		try {
			...
			change.addError("Class not found: " + className + " (" + e.toString() + ")");
			return;
		}
		// (PO po, UIField field, String oldValue, String newValue)
		Method method = null;
		try {
			method = clazz.getMethod(methodName, PO.class, UIField.class, String.class, String.class);
			...
			change.addError("Callout error: " + cmd + " (" + e.toString() + ")");
		}
	 ...
	 * @param newValue
	 */
	private void processCalloutDirect(ChangeVO change, PO po, int windowNo, UIField field, String oldValue,
			String newValue) {
		Method method = null;
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("UITab[").append(getAD_Tab_ID()).append("-").append(getName());
		if (p_vos != null) {
		...
		for (UIField f : getFields()) {
			...
			if (f.isLookup() && f.isDependentValue()) {
				...
				if (change.changedFields.containsKey(colName)) {
					...
					ArrayList<NamePair> arry = change.changedDropDowns.get(colName);
					ArrayList<NamePair> data = f.getAllLookupData(newCtx, windowNo);
					if (arry == null) {

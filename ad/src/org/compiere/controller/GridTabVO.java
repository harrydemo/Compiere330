...
public class GridTabVO implements Evaluatee, Serializable {
	 ...
	 * @return TabVO
	 */
	public static GridTabVO create(GridWindowVO wVO, int TabNo, ResultSet rs, boolean isRO, int onlyCurrentDays,
			int AD_UserDef_Win_ID, Trx p_trx) {
	...
	private static boolean loadTabDetails(GridTabVO vo, ResultSet rs) {
		...
		try {
			...
			if (rs.getString("IsTranslationTab").equals("Y")) {
				...
				if (!showTrl) {
					CLogger.get()
							.config("TrlTab Not displayed - AD_Tab_ID=" + vo.AD_Tab_ID + "=" + vo.Name + ", Table="
									+ vo.TableName + ", BaseTrl=" + Env.isBaseTranslation(vo.TableName)
	...
	private static boolean createFields(GridTabVO mTabVO, int AD_UserDef_Win_ID, Trx p_trx) {
		...
		try {
			...
			while (rs.next()) {
				GridFieldVO voF = GridFieldVO.create(mTabVO.ctx, mTabVO.WindowNo, mTabVO.TabNo, mTabVO.AD_Window_ID,
						AD_Tab_ID, mTabVO.IsReadOnly, rs);
				if (voF != null) {
	...
	public GridTabVO clone(Ctx myCtx, int windowNo) {
		...
		for (int i = 0; i < Fields.size(); i++) {
			GridFieldVO field = Fields.get(i);
			GridFieldVO cloneField = field.clone(myCtx, windowNo, TabNo, AD_Window_ID, AD_Tab_ID, IsReadOnly);
			if (cloneField == null) {
				return null;
			}

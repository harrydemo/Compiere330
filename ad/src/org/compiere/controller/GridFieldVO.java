...
public class GridFieldVO extends FieldVO implements Serializable {
	 ...
	 * @return MFieldVO
	 */
	public static GridFieldVO create(Ctx ctx, int WindowNo, int TabNo, int AD_Window_ID, int AD_Tab_ID,
	...
	public static GridFieldVO createParameter(GridFieldVO voF) {
		GridFieldVO voT = new GridFieldVO(voF.ctx, voF.WindowNo, voF.TabNo, voF.AD_Window_ID, voF.AD_Tab_ID,
				voF.tabReadOnly);
		voT.isProcess = true;
	 ...
	 * @return MFieldVO
	 */
	public static GridFieldVO createStdField(Ctx ctx, int WindowNo, int TabNo, int AD_Window_ID, int AD_Tab_ID,
			boolean tabReadOnly, boolean isCreated, boolean isTimestamp) {
	...
	protected void initFinish() {
		// Not null fields
		DisplayLogic = DisplayLogic == null ? "" : DisplayLogic;
		DefaultValue = DefaultValue == null ? "" : DefaultValue;
		FieldGroup = FieldGroup == null ? "" : FieldGroup;
		Description = Description == null ? "" : Description;
		Help = Help == null ? "" : Help;
		Callout = Callout == null ? "" : Callout;
		ReadOnlyLogic = ReadOnlyLogic == null ? "" : ReadOnlyLogic;
		mandatoryLogic = mandatoryLogic == null ? "" : mandatoryLogic;

		// Create Lookup, if not ID
		if (FieldType.isLookup(displayType)) {
			try {
				lookupInfo = MLookupFactory.getLookupInfo(ctx, WindowNo, displayType, AD_Column_ID,
	 ...
	 */
	protected GridFieldVO clone(Ctx Ctx, int windowNo, int tabNo, int ad_Window_ID, int ad_Tab_ID,
			boolean TabReadOnly) {
	...
	public String toString() {
		...
		sb.append(AD_Column_ID).append("-").append(ColumnName).append("]");
		return sb.toString();
	} // toString

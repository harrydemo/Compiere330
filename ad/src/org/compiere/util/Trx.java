...
/**
 * Transaction Management. - Create new Transaction by Trx.get(name); -
 * ..transactions.. - commit(); ---- start(); ---- commit(); - close();
 * 
 * @author Jorg Janke
 * @version $Id$
 */
public class Trx {
	...
	/**
	 * Create new transaction. You MUST call the close() method on the returned Trx
	 * object when you are finished with the transaction.
	 * 
	 * @param trxName Transaction Name prefix. Use for debugging purposes. Can be
	 ...
	 * rollback the transaction. You MUST manually commit or rollback before calling
	 * this method.
	 * 
	 * @return true if success
	 */
	...
	protected void finalize() throws Throwable {
		...
		if (m_conn != null && !m_conn.isClosed()) {
			log.log(Level.INFO,
					"Connection not properly closed; closing now; was borrowed at " + DB.getBorrower(m_conn));
			close();
		}
		super.finalize();
	}

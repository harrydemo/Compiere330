...
public final class DB {
	...
	public static void updateMail() {
		...
		sql = new StringBuffer("UPDATE AD_User SET ").append(" EMail=").append(DB.TO_STRING(adminEMail))
				...
				.append(DB.TO_STRING(mailUser)).append(" WHERE AD_User_ID IN (0,100)");
	...
	public static boolean isDatabaseOK(Ctx ctx) {
		...
		int no = JOptionPane.showOptionDialog(null, msg, title, JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE,
	...
	public static int executeUpdate(String sql, Object[] params, boolean ignoreError, Trx trx) {
		...
		//
		int no = -1;
		CPreparedStatement cs = new CPreparedStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE, sql,
				trx); // converted in call
	...
	private static int executeTrigger(String sql, Trx trx) throws SQLException {
		...
		if (no > -1) {
			...
			if (iret > -1) {
				...
				log.warning("      but unable to log for " + newSQL + localTrx == null ? " - Connection is null."
						: " - " + localTrx.toString());
			}
			localTrx.close();
	...
	public static RowSet getRowSet(String sql, boolean local) {
		...
		CStatementVO info = new CStatementVO(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY, sql);
	...
	public static int getSQLValue(Trx trx, String sql, Object[] params) {
		...
		CPreparedStatement cs = new CPreparedStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY, sql,
				trx);
	...
	public static int getSQLValue(Trx trx, String sql, int int_param1, int int_param2) {
		...
		try {
			...
			log.log(Level.SEVERE, sql + " - Param1=" + int_param1 + ",Param2=" + int_param2 + " [" + trx + "]", e);
		} finally {
			try {
				if (pstmt != null) {
					pstmt.close();
	 ...
	 * @return first value or -1
	 */
	public static int getSQLValue(Trx trx, String sql, int int_param1, int int_param2, String s_param3) {
		int retValue = -1;
		PreparedStatement pstmt = null;
		try {
			pstmt = prepareStatement(sql, trx);
			...
			if (rs.next()) {
				...
				log.info("No Value: " + sql + " - Param1=" + int_param1 + ",Param2=" + int_param2 + ", Param3="
						+ s_param3);
			}
			rs.close();
			pstmt.close();
			...
			log.log(Level.SEVERE, sql + " - Param1=" + int_param1 + ",Param2=" + int_param2 + ",Param3=" + s_param3, e);
		} finally {
			try {
				if (pstmt != null) {
					pstmt.close();
	...
	public static boolean isSOTrx(String TableName, String whereClause) {
		...
		try {
			...
			if (TableName.endsWith("Line")) {
				...
				sql = "SELECT IsSOTrx FROM " + hdr + " h WHERE EXISTS (SELECT * FROM " + TableName + " l WHERE h." + hdr
						+ "_ID=l." + hdr + "_ID AND " + whereClause + ")";
	...
	public static int getNextID(int AD_Client_ID, String TableName, Trx trx) {
		...
		// jz let trx = null so that the previous workflow node wouldn't block startnext
	 ...
	 */
	public static String getDocumentNo(Ctx ctx, int WindowNo, String TableName, boolean onlyDocType, Trx trx) {
		...
		if (C_DocType_ID == 0) {
			log.fine("Window=" + WindowNo + " - Target=" + ctx.getContextAsInt(WindowNo + "|C_DocTypeTarget_ID") + "/"
					+ ctx.getContextAsInt(WindowNo, "C_DocTypeTarget_ID") + " - Actual="
					+ ctx.getContextAsInt(WindowNo + "|C_DocType_ID") + "/"
					+ ctx.getContextAsInt(WindowNo, "C_DocType_ID"));
		...
		while (iter.hasNext()) {
			...
			try {
				if (entry.getKey().isClosed()) {
					...
					log.log(Level.WARNING, "Connection[" + i + "]=" + entry.getKey() + " borrowed from: ",
							entry.getValue());
					++i;
				}
			} catch (SQLException e) {
	...
	static void returnCachedConnection(Connection conn) {
		...
		if (t == null) {
			log.log(Level.WARNING,
					"Connection=" + conn + " was not previously borrowed; returned at " + getBorrowPoint());
		}

...
import org.compiere.common.constants.CtxConstants;
...
public class Login {
	...
	/**
	 * Attempt to change roles using a previously authenticated Login. This function
	 * should only be called after a successful login.
	 * 
	 ...
	 * @param app_pwd    pwd
	 * @param force      ignore pwd
	 * @param ignore_pwd If true, indicates that the user had previously
	 *                   authenticated successfully, and therefore there is no need
	 *                   to check password again. This differs from the <b>force</b>
	 *                   parameter in that <b>force</b> will force a login with
	...
	/**
	 * Get Roles for the user with email in client with the web store. If the user
	 * does not have roles and the web store has a default role, it will return
	 * that.
	 * 
	...
	private KeyNamePair[] getRolesByEmail(String eMail, String password, int W_Store_ID) {
		...
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
		//
		String sql = "SELECT u.AD_User_ID, r.AD_Role_ID, u.Name " + "FROM AD_User u"
				+ " INNER JOIN W_Store ws ON (u.AD_Client_ID=ws.AD_Client_ID) "
				+ " INNER JOIN AD_Role r ON (ws.AD_Role_ID=r.AD_Role_ID) " + "WHERE u.EMail=?"
				+ " AND (u.Password=? OR u.Password=?)" + " AND ws.W_Store_ID=?"
				+ " AND (r.IsActive='Y' OR r.IsActive IS NULL)" + " AND u.IsActive='Y' AND ws.IsActive='Y'"
				+ " AND u.AD_Client_ID=ws.AD_Client_ID " + "ORDER BY r.Name";
		m_roles.clear();
		m_users.clear();
		PreparedStatement pstmt = null;
		try {
	...
	public KeyNamePair[] getOrgs(KeyNamePair client) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (summary) {
					if (m_roleModel == null || m_roleModel.getAD_Role_ID() != AD_Role_ID
	...
	public KeyNamePair[] getWarehouses(KeyNamePair org) {
		...
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
		KeyNamePair[] retValue = null;
		String sql = "SELECT M_Warehouse_ID, Name FROM M_Warehouse " + "WHERE AD_Org_ID=? AND IsActive='Y' "
				+ "ORDER BY Name";
		PreparedStatement pstmt = null;
		try {
			pstmt = DB.prepareStatement(sql, (Trx) null);
	...
	public MSession createSwingSession() {
		...
		MSession session = MSession.get(m_ctx, X_AD_Session.SESSIONTYPE_SwingUI, true, Remote_Addr, Remote_Host, null);
		return session;
	} // createSwingSession
	...
	public String validateLogin(KeyNamePair org) {
		String info = m_user + ",R:" + m_role.toStringX() + ",O=" + org.toStringX();
	...
	public String loadPreferences(KeyNamePair org, KeyNamePair warehouse, java.sql.Timestamp timestamp,
		...
		String sql = "SELECT a.C_AcctSchema_ID, a.C_Currency_ID, a.HasAlias, c.ISO_Code, c.StdPrecision "
				+ "FROM C_AcctSchema a" + " INNER JOIN AD_ClientInfo ci ON (a.C_AcctSchema_ID=ci.C_AcctSchema1_ID)"
				+ " INNER JOIN C_Currency c ON (a.C_Currency_ID=c.C_Currency_ID) " + "WHERE ci.AD_Client_ID=?";
		PreparedStatement pstmt = null;
		try {
			...
			// Last login date
			sql = "SELECT CREATED " + "FROM AD_Session " + "WHERE AD_Session_ID=?";
			pstmt = DB.prepareStatement(sql, (Trx) null);
	...
	private String loadDefault(String TableName, String ColumnName) {
		if (TableName.startsWith("AD_Window") || TableName.startsWith("AD_PrintFormat") || TableName.equals("AD_Tree")
		...
		sql = m_roleModel.addAccessSQL(sql, TableName, MRole.SQL_NOTQUALIFIED, MRole.SQL_RO);
		PreparedStatement pstmt = null;
		try {
			pstmt = DB.prepareStatement(sql, (Trx) null);
			ResultSet rs = pstmt.executeQuery();
	...
	public Locale setLanguage(String langInfo) {
		...
		// we get locale here before language verification. In case when laguange is not
		// installed, we can still get the current locale

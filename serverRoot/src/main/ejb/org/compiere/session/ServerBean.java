 ...
 *
 * @ejb.bean name="compiere/Server" display-name="Compiere Server Session Bean"
 *           type="Stateless" view-type="both" transaction-type="Bean"
 *           jndi-name="compiere/Server" local-jndi-name="compiere/ServerLocal"
 *
 * @ejb.ejb-ref ejb-name="compiere/Server" view-type="both"
 *              ref-name="compiere/Server"
 * @ejb.ejb-ref ejb-name="compiere/Server" view-type="local"
...
public class ServerBean implements SessionBean {
	 ...
	 * @param AD_Window_ID the internal number of the window, if not 0, AD_Menu_ID
	 *                     is ignored
	 ...
	 */
	public String postImmediate(Ctx ctx, int AD_Client_ID, int AD_Table_ID, int Record_ID, boolean force, Trx trx) {
	...
	public RowSet pstmt_getRowSet(CStatementVO info) throws NotSerializableException {
		...
		try {
			...
			RowSet remote_getRowSet = pstmt.remote_getRowSet();
			pstmt.close();
			return remote_getRowSet;
	...
	public RowSet stmt_getRowSet(CStatementVO info) {
		...
		RowSet remote_getRowSet = stmt.remote_getRowSet();
		try {
			if (stmt != null)
				stmt.close();
	...
	public int stmt_executeUpdate(CStatementVO info) {
		...
		m_stmt_updateCount++;
		CPreparedStatement pstmt = new CPreparedStatement(info);
		int remote_executeUpdate = pstmt.remote_executeUpdate();
		try {
			if (pstmt != null) {
				pstmt.close();
			}
	 ...
	 * 
	 * @ejb.interface-method view-type="both" Called from MPayment processOnline
	 * @param ctx                   Context
	 ...
	 */
	public boolean paymentOnline(Ctx ctx, int C_Payment_ID, int C_PaymentProcessor_ID, Trx trx) {
	 ...
	 */
	public EMail createEMail(Ctx ctx, int AD_Client_ID, int AD_User_ID, String toEMail, String toName, String subject,
			String message) {
		MClient client = MClient.get(ctx, AD_Client_ID);
	...
	public String getStatus() {
		...
		sb.append(m_no).append("-Window=").append(m_windowCount).append(",Post=").append(m_postCount)
				.append(",Process=").append(m_processCount).append(",NextSeq=").append(m_nextSeqCount)
				.append(",Workflow=").append(m_workflowCount).append(",Payment=").append(m_paymentCount)
				.append(",RowSet=").append(m_stmt_rowSetCount).append(",Update=").append(m_stmt_updateCount)
				.append(",CacheReset=").append(m_cacheResetCount).append(",SvrRetart=").append(m_restartCount)
				.append(",UpdateLob=").append(m_updateLOBCount).append("]");
		return sb.toString();
	} // getStatus
	...
	protected static void dumpSVUID(Class clazz) // XDoclet: no generics
	{
		String s = clazz.getName() + " ==\nstatic final long serialVersionUID = "
				+ java.io.ObjectStreamClass.lookup(clazz).getSerialVersionUID() + "L;\n";
		System.out.println(s);
	} // dumpSVUID

	/**
	 * Print UID of used classes. R2.5.1h
	 * 
	 * org.compiere.process.ProcessInfo == static final long serialVersionUID =
	 * -1993220053515488725L;
	 * 
	 * org.compiere.util.CStatementVO == static final long serialVersionUID =
	 * -3393389471515956399L;
	 * 
	 * org.compiere.model.MQuery == static final long serialVersionUID =
	 * 1511402030597166113L;
	 * 
	 * org.compiere.model.POInfo == static final long serialVersionUID =
	 * -5976719579744948419L;
	 * 
	 * org.compiere.model.POInfoColumn == static final long serialVersionUID =
	 * -3983585608504631958L;
	 * 
	 * org.compiere.model.MWindowVO == static final long serialVersionUID =
	 * 3802628212531678981L;
	 * 
	 * org.compiere.model.MTabVO == static final long serialVersionUID =
	 * 9160212869277319305L;
	 * 
	 * org.compiere.model.MFieldVO == static final long serialVersionUID =
	 * 4385061125114436797L;
	 * 
	 * org.compiere.model.MLookupInfo == static final long serialVersionUID =
	 * -7958664359250070233L;
	 *
	 * 
	 * * @param args ignored
	 */
